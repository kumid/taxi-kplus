import { Image, StyleSheet, Platform, SafeAreaView, FlatList, TouchableOpacity, View, Text, Dimensions, Alert } from 'react-native';
...
import UpdateLoanCard from '@/components/UpdateLoanCard';
import ConfirmationDialog from '@/components/ConfirmationDialog';
import UpdateLoanCardMobile from '@/components/UpdateLoanCardMobile';
...
export default function CardsScreen() {
  const { cachedCards, loadingCards, updateCards, updateCardsResult, deleteCard } = useDataContext();
  const { width } = Dimensions.get('window'); // Get the screen width
  const columnCount = Platform.OS == 'web' ? 4 : 1; // Number of columns per row
  const cardWidth = (width - (columnCount - 1) * 10) / columnCount - (Platform.OS == 'web' ? 0 : 30); // Calculate card width with margin space
  ...
  const itemDeleteAction = async () => {
    await deleteCard(offerToDelete);
    setIsConfirmationDialogVisible(false)
  };


  useEffect(() => {
    if (updateCardsResult.success === true) {
  ...
  useEffect(() => {
    ...
    const lst = cachedCards?.filter((card) => card.lang === selectedLang);
    setFilteredCards(lst)
  }, [cachedCards, selectedLang])
    ...
    <>
      <View style={{ flex: 1, padding: 15 }}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.notificationIcon} onPress={toggleAddDialogModal}>
          ...
          <View style={styles.row}>
            {['ru', 'en', 'es', 'vi'].map(value => (
              <TouchableOpacity
                ...
                style={[styles.button, selectedLang === value && styles.selected]}>
                <Text
                  style={[
                    styles.buttonLabel,
      ...
      <Dialog isVisible={isAddDialogVisible} onClose={toggleAddDialogModal} dialogWidth={dialogSize} scrollable={true}>
        {
          Platform.OS === 'web' ?
            <UpdateLoanCard element={offerToDialog} updateElement={(element) => updateCards(element)} /> :
            <UpdateLoanCardMobile element={offerToDialog} updateElement={(element) => updateCards(element)} />
        }

      </Dialog>

      <Dialog isVisible={isConfirmationDialogVisible} onClose={() => setIsConfirmationDialogVisible(false)} dialogWidth={dialogSize}  scrollable={false}>
