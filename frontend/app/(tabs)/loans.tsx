...
export default function LoansScreen() {
  const { cachedLoans, loadingLoans, updateLoans, updateLoansResult, deleteLoan  } = useDataContext();
  ...
  const itemDeleteAction = async () => {
    await deleteLoan(offerToDelete);
    setIsConfirmationDialogVisible(false)
  };
  

  useEffect(() => {   
    if(updateLoansResult.success === true) {
  ...
  useEffect(() => {
    const lst = cachedLoans.filter((card) => card.lang === selectedLang);
    setFilteredCards(lst)
  }, [cachedLoans, selectedLang])

  const toggleAddDialogModal = (): void => {
    setOfferToDialog({...sampleCard,
      id: 0,
      lang: selectedLang
     });
    setIsAddDialogVisible(!isAddDialogVisible);
    ...
    <>
      <View style={{ flex: 1, padding: 15 }}> 
        <View style={styles.header}>
          <View>
            <Text style={styles.welcome}>Займы</Text>
          </View>
      ...
      <Dialog isVisible={isAddDialogVisible} onClose={toggleAddDialogModal} dialogWidth={dialogSize}  scrollable={true}>
          <UpdateLoanCard element={offerToDialog} updateElement={(element) => updateLoans(element)}/>
      </Dialog>
